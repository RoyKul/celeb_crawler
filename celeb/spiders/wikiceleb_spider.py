...
class WikiCelebsSpider(scrapy.Spider):
    name = 'wikiceleb'
    allowed_domains = ['wikiofcelebs.com']
    start_urls = ['http://wikiofcelebs.com/']
    celeb_form = re.compile("https:\/\/www.wikiofcelebs.com\/\d+\/\d+\/\w+-\w+.+\.html")
    regex_form ="\/\w+-\w+"
    crawled_pages = set()
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        ...
        Returns:
            [type] -- [description]
        """
        spider = super(WikiCelebsSpider, cls).from_crawler(crawler, *args, **kwargs)
