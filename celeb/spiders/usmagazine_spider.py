...
class UsmagazineSpider(scrapy.Spider):
    name = 'usmagazine'
    allowed_domains = ['usmagazine.com'] # If allowed domain is like start_url the crawling ends quickly but only  1/3 of the celebs appear
    # If allowed domain is 'http://usmagazine.com' then the crawling is extremly long but results are good
    start_urls = ['http://usmagazine.com/celebrities']
    celeb_form = re.compile("https:\/\/www.usmagazine.com\/celebrities\/\w+-\w+\/")
    crawled_pages = set()
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        ...
        Returns:
            [type] -- [description]
        """
        spider = super(UsmagazineSpider, cls).from_crawler(crawler, *args, **kwargs)
