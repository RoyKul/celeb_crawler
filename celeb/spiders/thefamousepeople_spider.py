...
class ThefamouspeopleSpider(scrapy.Spider):
    name = 'thefamouspeople'
    allowed_domains = ['thefamouspeople.com'] 
    start_urls = ['http://thefamouspeople.com/profiles']
    celeb_form = re.compile("https:\/\/www\.thefamouspeople\.com\/profiles\/(\w+-){1,4}(\d)*.php")
    regex_form ="((\w+-){1,3})\w+"
    crawled_pages = set()
    ...
    def from_crawler(cls, crawler, *args, **kwargs):
        ...
        Returns:
            [type] -- [description]
        """
        spider = super(ThefamouspeopleSpider, cls).from_crawler(crawler, *args, **kwargs)
    ...
    def parse_url(self,url):
        ...
        celeb = re.sub("-"," ",celeb.group(1)).title()
